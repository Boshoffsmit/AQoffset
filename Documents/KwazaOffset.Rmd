...
df <- data.frame((cellStats(hh_pm10_exceed, sum) / (nrow(hh_pm10_exceed) * ncol(hh_pm10_exceed) * nlayers(hh_pm10_exceed)) ) , stringsAsFactors = FALSE)
...
\pagebreak
The baseline burden of disease impact for SO2 in the baseline scenario is shown below.
```{r echo = FALSE, message=FALSE}
...
```{r echo = FALSE}
raster_dist_plot(hh_24 * 0.49, multi = c("pm10", "so2"), th=BuRdTheme) + addkwa()
```
